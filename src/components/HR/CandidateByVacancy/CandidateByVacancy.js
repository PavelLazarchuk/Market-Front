...
const CandidateByVacancy = ({ t, vacancy, getOneVacancy, getSuitableCandidates }) => {
		...
		<PageWrap title={t('CANDIDATE')}>
			{vacancy && (
				<div>
					...
					<Text className={classes.textCenter}>{arrToStringSkill(vacancy.skills)}</Text>
				</div>
			)}
			<HRVacancyMenu classes={classes} id={id} />
			<div className={classes.hrVacancyRoutes}>
				<Route exact path="/hr/vacancy/:id" render={() => <CandidateList id={id} />} />
